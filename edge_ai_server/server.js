...
io.on('connection', (socket) => {
  ...
  // Dashboard → junction command
  socket.on('cmd', ({ junction, cmd }) => {
    if (junction && cmd && JUNCTIONS[junction]) {
      mqttPublish(`smartjunction/${junction}/cmd`, String(cmd).toUpperCase());
      console.log(`[WS CMD] ${junction} ← ${cmd}`);
    }
  });
